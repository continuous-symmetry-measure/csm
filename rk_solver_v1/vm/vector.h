...
template <class T>
Vector<typename VMTraits<T>::real_type> Vector<T>::real() const
{
...
template <class T>
Vector<typename VMTraits<T>::real_type> Vector<T>::imag() const
{
	Vector<real_type> tmp(nn_);
	real_type *p1 = tmp.data();
	T  *p2 = ptr_;
...
{
	return typename Vector<T>::iterator(ptr_, stride_);
}

template <class T>
inline typename Vector<T>::const_iterator Vector<T>::begin() const
{
	return typename Vector<T>::const_iterator(ptr_, stride_);
}

template <class T>
inline typename Vector<T>::iterator Vector<T>::end()
{
	return typename Vector<T>::iterator(ptr_ + nn_ * stride_, stride_);
}

template <class T>
inline typename Vector<T>::const_iterator Vector<T>::end() const
{
	return typename Vector<T>::const_iterator(ptr_ + nn_ * stride_, stride_);
}
...
template <class T>
double scalarMax(const Vector<T>& vec)
{
	
	if (!vec.size()) vmerror("Maximum of zero size Vector not allowed.");
	double tmp = vec[0].max();	
	for (size_t i = 1; i < vec.size(); ++i)
	{
		double vMax = (vec[i]).max();
...
template <> 
double scalarMax(const Vector<double>& vec) 
{
	if (!vec.size()) vmerror("Maximum of zero size Vector not allowed.");
	double tmp = vec[0];
	for (size_t i = 1; i < vec.size(); ++i)
