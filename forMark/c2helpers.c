...
char* operationName = "C2 SYMMETRY";
...
/*
 * c2_estimation in this case of c2
 */
double chiralityFunction(double *parray[], double *anti_parray[],
  ...
  *d_min=0.0;
  ...
  //*d_min = (2.0*eigen_val_max-*d_min)/(double)num_of_atoms;
  *d_min = (2.0*eigen_val_max-*d_min);
                    ...
                    double dir_cos[])
{
  ...
  for(atom = 0; atom < num_of_atoms; ++atom)
     ...
     *eigen_val_max = -1.0e10;
    for (i=0; i<3; i++) {
       *eigen_val_max = (*eigen_val_max > eigen_val[i]) ?
   ...
   for (i=0; i<3; i++)
    ...
    for (i=0; i<3; i++)
       tmp[i][i] = coef[i][i] - *eigen_val_max;
