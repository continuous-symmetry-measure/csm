...
from csm.input_output.formatters import format_CSM, non_negative_zero, format_unknown_str, \
    output_strings
...
class ScriptWriter:
    ...
    def create_approx_statistics(self):
        ...
        for mol_index, mol_results in enumerate(self.results):
            for line_index, command_result in enumerate(mol_results):
                name = command_result.molecule.metadata.header(no_file_format=True) + "_" + get_line_header(line_index,
                                                                                                            command_result)
    ...
    def create_legacy_files(self):
        ...
        for mol_index, mol_results in enumerate(self.results):
            for line_index, command_result in enumerate(mol_results):
                ...
                try:
                    of = OldFormatFileWriter(command_result, out_file_name,
                                             out_format=command_result.molecule.metadata.format,
                                             print_local=self.print_local)
