    ...
    def find_best_perm(self):
        ...
        if self._op_type == 'CI' or (self._op_type == 'SN' and self._op_order == 2):
            ...
            for chainperm in self._chain_permutations:
                ...
                best_for_chain_perm = csm_operation(self._op_type, self._op_order, self._molecule, keep_structure=False, perm=perm)
                if best_for_chain_perm.csm < best.csm:
                    best = best_for_chain_perm
        else:
            if self._print_approx:
                print("There are", len(self._dirs), "initial directions to search for the best permutation")
            for chainperm in self._chain_permutations:
                if self._print_approx:
                    print("Calculating for chain permutation ", chainperm)
                for dir in self._dirs:
                    ...
                    if self._print_approx:
                        ...
                        print("\t\tfirst pass yielded dir", interim_results.dir,
                    ...
                    while (i < max_iterations and
                        ...
                        if self._print_approx:
                            ...
                            print("\t\t\tthe distance between the new direction and the previous direction is:",
