...
public:
	...
	Matrix operator~() const;
	typename Vector<T>::iterator operator[](size_t i);
	typename Vector<T>::const_iterator operator[](size_t i) const;
	...
	typename Vector<T>::iterator rbegin(size_t n);
	typename Vector<T>::const_iterator rbegin(size_t n) const;
	typename Vector<T>::iterator rend(size_t n);
	typename Vector<T>::const_iterator rend(size_t n) const;
	typename Vector<T>::iterator cbegin(size_t n);
	typename Vector<T>::const_iterator cbegin(size_t n) const;
	typename Vector<T>::iterator cend(size_t n);
	typename Vector<T>::const_iterator cend(size_t n) const;
	...
	T max() const
	{
		if (!mm_ || !nn_) vmerror("Can't compute imum of zero-sized matrix");
		size_t i, j;
		T *p1;
		T tmp = operator()(0,0);
		for (i = 0; i < nn_; ++i)
...
template <class T>
inline typename Vector<T>::iterator Matrix<T>::operator[](size_t i)
{
...
template <class T>
inline typename Vector<T>::const_iterator Matrix<T>::operator[](size_t i) const
{
#ifdef VM_BOUNDS_CHECK
	if (i >= nn_)
		vmerror("Index out of bounds error.");
...
template <class T>
Matrix<typename VMTraits<T>::real_type> Matrix<T>::real() const
{
...
template <class T>
Matrix<typename VMTraits<T>::real_type> Matrix<T>::imag() const
{
	Matrix<real_type> tmp(nn_, mm_);
	size_t i, j;
	real_type *p1 = tmp.data();
...
template <class T>
inline typename Vector<T>::iterator Matrix<T>::rbegin(size_t n)
{
...
template <class T>
inline typename Vector<T>::const_iterator Matrix<T>::rbegin(size_t n) const
{
	if (n >= nn_) vmerror("Index out of bounds error.");
...
template <class T>
inline typename Vector<T>::iterator Matrix<T>::rend(size_t n)
{
	if (n >= nn_) vmerror("Index out of bounds error.");
...
template <class T>
inline typename Vector<T>::const_iterator Matrix<T>::rend(size_t n) const
{
	if (n >= nn_) vmerror("Index out of bounds error.");
...
template <class T>
inline typename Vector<T>::iterator Matrix<T>::cbegin(size_t n)
{
...
template <class T>
inline typename Vector<T>::const_iterator Matrix<T>::cbegin(size_t n) const
{
	if (n >= mm_) vmerror("Index out of bounds error.");
...
template <class T>
inline typename Vector<T>::iterator Matrix<T>::cend(size_t n)
{
	if (n >= mm_) vmerror("Index out of bounds error.");
...
template <class T>
inline typename Vector<T>::const_iterator Matrix<T>::cend(size_t n) const
{
	if (n >= mm_) vmerror("Index out of bounds error.");
...
template <>
Vector<Matrix<double> > Vector<Matrix<double> >::copy() const
{
	Vector<Matrix<double> > tmp;
	if (data_)
	{
		alloc_type a2;
		Matrix<double> *new_data = a2.allocate(nn_, 0);
		if (!new_data) vmerror("Memory allocation error in Vector::copy().");
		Matrix<double> *p1 = new_data;
		const Matrix<double> *p2 = ptr_;
		ptrdiff_t s = stride_;
		size_t i;
		for (i = 0; i < nn_; ++i)
		{
			a2.construct(p1, (*p2).copy());
			++p1;
			p2 += s;
		}
		tmp.data_      = new_data;
