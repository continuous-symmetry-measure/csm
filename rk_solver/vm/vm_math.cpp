...
Vector<typename VMTraits<T>::real_type> abs(const Vector<T>& x)
{
	using namespace std;
	size_t n = x.size();
	Vector<typename VMTraits<T>::real_type> tmp(n);
	for (size_t i = 0; i < n; ++i)
		tmp[i] = abs(x[i]);
	return tmp;
}
...
{
	using namespace std;
	Matrix<typename VMTraits<T>::real_type> tmp(x.nrows(), x.ncols());
	size_t n = tmp.nrows();
	size_t m = tmp.ncols();
	size_t i, j;
	for (i = 0; i < n; ++i)
...
template <class T>
Vector<Matrix<T> > operator*(const Vector<Matrix<T> > &lhs, const T &rhs) {
	Vector<Matrix<T> > res = lhs.copy();
